...
int print_tcp_hdr(WINDOW *win, unsigned char const *buf, int bufsize)
{
...
int print_tcp_packet(WINDOW *win, unsigned char const *buf, int bufsize)
{
	int hdrs_len = get_ethhdr_len() + get_iphdr_len(buf)
	               + get_tcphdr_len(buf);
	int pdata_size = bufsize - hdrs_len;
	unsigned char *pdata = (unsigned char*)buf + hdrs_len;
...
int print_udp_hdr(WINDOW *win, unsigned char const* buf, int bufsize)
{
...
int print_udp_packet(WINDOW *win, unsigned char const *buf, int bufsize)
{
